...
class AppointmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'MOTHER':
            return Appointment.objects.filter(mother__user=user) | Appointment.objects.filter(child__mother__user=user)
...
class ANCVisitViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        if user.role == 'MOTHER':
            return ANCVisit.objects.filter(pregnancy__mother__user=user)
        return ANCVisit.objects.all()
